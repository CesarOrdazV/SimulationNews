...
storage.py                     # seen.json / latest_by_topic.json load/save
collector.py                   # Feed-group iteration, deduplication, display fallback
...
data/
  ...
  latest_by_topic.json         # Last relevant article per section:topic (fallback)
docs/
...
2. **Load** `data/latest_by_topic.json` — dict `{ "section:topic": { article fields... } }`.
3. **Iterate two feed groups:**
...
4. **For each entry:** discard if title+summary contain none of `KEYWORDS`; update `latest_by_topic` when the article is newer; skip for "new" if the link is already in `seen`.
5. **Normalize timestamps** via `datetime_utils.py` (`America/Mexico_City`, format `YYYY-MM-DD HH:MM`) for entry `published` and report generation time.
6. **Build display rows** with `build_display_items()`:
   - topics with new articles → show **only** those new articles
   - topics with no new articles → show the stored last article from `latest_by_topic.json` (marked as fallback)
7. **Write outputs** from the display rows (not a cumulative history of all past articles).
8. **Persist** the updated `seen.json` and `latest_by_topic.json`.

## Date and time

- All user-facing timestamps use **`America/Mexico_City`** (stdlib `zoneinfo`; no extra deps).
- Helpers live in `datetime_utils.py`: `format_mexico_now()`, `format_entry_published()`, `format_mexico_datetime()`, `is_published_newer()`.
...
| File | Format | Content |
|------|--------|---------|
| `business_intelligence_brief.md` | Markdown | Header `Generated: … (Mexico City)`; Business Keywords / Competitors sections, grouped by topic; fallback rows marked _(last seen)_ |
...
| `docs/index.html` | Static HTML | Header `Updated: … (Mexico City)`; same data with dark-mode UI; fallback rows labeled "Last seen" |
| `data/seen.json` | JSON | Deduplication state (mutated on every run) |
| `data/latest_by_topic.json` | JSON | Newest relevant article per topic (mutated on every run) |

If a topic has neither new articles nor a stored latest article, it is omitted. If the whole brief is empty, reports say *"No relevant articles available."*
...
- **Auto-commit** of: `business_intelligence_brief.md`, `.csv`, `data/seen.json`, `data/latest_by_topic.json`, `docs/index.html`
...
1. **Running the script mutates tracked files.** If you only test the environment, restore afterward:
   ...
   ```
2. **`seen.json` already has extensive history** — a normal run often reports `0 new posts`, but topics still show their last article via `latest_by_topic.json`. To force a full "new" dump, temporarily reset seen (`echo "{}" > data/seen.json`), run, then restore (or use `reset_seen` in Actions).
...
| Change collection / dedup / fallback logic | `collector.py` |
